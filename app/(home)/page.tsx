...
export default async function Home() {
  ...
  const subscribedResponse = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/user`, {
    method : "GET",
  ...
  const subscribedCommunityNames = userData.subscribedCommunities.map((community : UserSubscribedCommunity) => community.name)
  ...
  const subscribedPosts = posts.filter((post : Post) => subscribedCommunityNames.includes(String(post.goalName)))
